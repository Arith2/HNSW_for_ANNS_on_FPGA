...
// read binary file
    ...
    // size_t HBM_query_size = 10000 * 128;
...
// allocate aligned 2D vectors
    ...
    std::vector<uint32_t, aligned_allocator<uint32_t>> HBM16_data(HBM_data_size, 0);
    ...
    // std::vector<uint32_t, aligned_allocator<uint32_t>> HBM29_data(HBM_query_size, 0);
    // std::vector<uint32_t, aligned_allocator<uint32_t>> HBM30_data(HBM_query_size, 0);
    // std::vector<uint32_t, aligned_allocator<uint32_t>> HBM31_data(HBM_query_size, 0);
...
// store data
    ...
    for (int i = 0; i < (int) cur_element_count; i++) {
        ...
        for (int j = 0; j < 2; j++) {
            for (int k = 0; k < 16; k++) {
                uint32_t data_0 = Byte2Float(data_level0_memory_ +  i * size_data_per_element_ + offsetData_ + 256 * j + 4 * k);
                uint32_t data_1 = Byte2Float(data_level0_memory_ +  i * size_data_per_element_ + offsetData_ + 256 * j + 4 * k + 64);
                uint32_t data_2 = Byte2Float(data_level0_memory_ +  i * size_data_per_element_ + offsetData_ + 256 * j + 4 * k + 128);
                uint32_t data_3 = Byte2Float(data_level0_memory_ +  i * size_data_per_element_ + offsetData_ + 256 * j + 4 * k + 192);
                HBM0_data[i * 32 + 16 * j + k] = data_0;
                HBM1_data[i * 32 + 16 * j + k] = data_1;
                HBM2_data[i * 32 + 16 * j + k] = data_2;
                HBM3_data[i * 32 + 16 * j + k] = data_3;

                HBM5_data[i * 32 + 16 * j + k] = data_0;
                HBM6_data[i * 32 + 16 * j + k] = data_1;
                HBM7_data[i * 32 + 16 * j + k] = data_2;
                HBM8_data[i * 32 + 16 * j + k] = data_3;

                HBM10_data[i * 32 + 16 * j + k] = data_0;
                HBM11_data[i * 32 + 16 * j + k] = data_1;
                HBM12_data[i * 32 + 16 * j + k] = data_2;
                HBM13_data[i * 32 + 16 * j + k] = data_3;

                HBM15_data[i * 32 + 16 * j + k] = data_0;
                HBM16_data[i * 32 + 16 * j + k] = data_1;
                HBM17_data[i * 32 + 16 * j + k] = data_2;
                HBM18_data[i * 32 + 16 * j + k] = data_3;

                // HBM20_data[i * 32 + 16 * j + k] = data_0;
                // HBM21_data[i * 32 + 16 * j + k] = data_1;
                // HBM22_data[i * 32 + 16 * j + k] = data_2;
                // HBM23_data[i * 32 + 16 * j + k] = data_3;
            }
        }
        ...
        for (int j = 0; j < 32; j++) {
            int gnd_link = (j < gnd_size) ? Byte2Int(data_level0_memory_ +  i * size_data_per_element_ + 4 * (j + 1)) : 0x00ffffff;
            if (j==0) {
                gnd_link = (gnd_link & 0x00ffffff) | ((gnd_size & 0x000000ff) << 24);
            }
            HBM4_data[i * 32 + j] = gnd_link;

            HBM9_data[i * 32 + j] = gnd_link;

            HBM14_data[i * 32 + j] = gnd_link;

            HBM19_data[i * 32 + j] = gnd_link;

            // HBM24_data[i * 32 + j] = gnd_link;
        }
    }
...
#else 
            int index = i % 2;
            uint32_t query_data = Byte2Float(query_memory_ + index * size_data_per_query + 4 + 4 * j);
#endif
            HBM25_data[index_base + j] = query_data;
// #ifndef DEBUG
//             if (i%4==0) {
...
//             }
//             if (i%4==2) {
//                 HBM29_data[index_base * 128 + j] = query_data;
//             }
//             else {
//                 HBM30_data[index_base * 128 + j] = query_data;
//             }
// #else 
//             if (i%4==0) {
//                 HBM27_data[index_base * 128 + j] = query_data;
...
//////////////////////////////   TEMPLATE START  //////////////////////////////
    ...
    OCL_CHECK(err, err = krnl_vector_add.setArg(16, buffer_HBM16_data));
    ...
    OCL_CHECK(err, err = krnl_vector_add.setArg(20, buffer_HBM25_data));
    // OCL_CHECK(err, err = krnl_vector_add.setArg(21, buffer_HBM26_data));
    // OCL_CHECK(err, err = krnl_vector_add.setArg(22, buffer_HBM27_data));
    // OCL_CHECK(err, err = krnl_vector_add.setArg(23, buffer_HBM28_data));
    // OCL_CHECK(err, err = krnl_vector_add.setArg(24, buffer_HBM29_data));
    // OCL_CHECK(err, err = krnl_vector_add.setArg(25, buffer_HBM30_data));
	...
	OCL_CHECK(err, err = krnl_vector_add.setArg(21, enterpoint_node_));
	OCL_CHECK(err, err = krnl_vector_add.setArg(22, (int)cur_element_count));
	OCL_CHECK(err, err = krnl_vector_add.setArg(23, maxlevel_));
    OCL_CHECK(err, err = krnl_vector_add.setArg(24, ef_));
    OCL_CHECK(err, err = krnl_vector_add.setArg(25, index_link));
    OCL_CHECK(err, err = krnl_vector_add.setArg(26, q_size));
//////////////////////////////   TEMPLATE END  //////////////////////////////
// ------------------------------------------------------
...
//////////////////////////////   TEMPLATE START  //////////////////////////////
    ...
    OCL_CHECK(
        ...
        buffer_HBM16_data, 
        buffer_HBM17_data, buffer_HBM18_data, buffer_HBM19_data, 
        // buffer_HBM20_data, buffer_HBM21_data, buffer_HBM22_data, buffer_HBM23_data, buffer_HBM24_data, 
        buffer_HBM25_data
        // , buffer_HBM26_data, buffer_HBM27_data
        // buffer_HBM28_data, 
        // buffer_HBM29_data, buffer_HBM30_data
        //  buffer_HBM31_data
        // buffer_DDR0_data
        }, 0/* 0 means from host*/));
