...
// #ifndef DEBUG
    ...
    //         ap_uint<512> tmp_data_0 = DDR0_data[total_link_row + 16*j + 2*k];
    //         ap_uint<512> tmp_data_1 = DDR0_data[total_link_row + 16*j + 2*k + 1];
    //         if (k < 4) {
...
void loadQuery(
    ...
    for (int j = 0; j < q_size; j++) {
        for (int k = 0; k < 4; k++) {
            #pragma HLS pipeline II=2
            ap_uint<512> tmp_data_0 = HBM_data[8*j + 2*k];
            ap_uint<512> tmp_data_1 = HBM_data[8*j + 2*k + 1];
        ...
        do {
            if (firstLoop | !ep_node_stream.empty()) {
                if (!firstLoop) {
                    ...
                    std::cout << "ep_node: " << meta_info_32(20, 1) << ", level: " << meta_info_32(23, 21) << ", channel id: " << channel_id << std::endl;
                }
                else {
                    ...
                    firstLoop = false;
                    std::cout << "firstLoop, channel id: " << channel_id << std::endl;
                }
            }
            else {
                stopLoop = false;
    ...
    do {
        if (!high_link_id_stream_array[0].empty() | !high_link_id_stream_array[1].empty()) {
            ...
            if (stop_flag==1) {
                ...
                switch (channel_id) {
                    case 0:  
                        std::cout << "  write meta into channel 0" << std::endl;
                        high_layer_links_stream_array[0].write(meta_info_512);
                        break;
                    default: 
                        std::cout << "  write meta into channel 1" << std::endl;
                        high_layer_links_stream_array[1].write(meta_info_512);
                        break;
                }
                ...
                switch (channel_id) {
                    case 0:  
                        std::cout << "  write links into channel 0" << std::endl;
                        high_layer_links_stream_array[0].write(high_layer_links);
                        break;
                    default: 
                        std::cout << "  write links into channel 1" << std::endl;
            ...
            if (meta_flag!=0xf) {
                ...
                if (node_id==0x00ffffff) {
                    ...
                    for (int k = 0; k < 4; k++) {
                        ...
                        ap_uint<512> hbm_tmp = HBM_data[4*node_id+k]; 
                        if (channel_id==0) {
                            data_stream_0.write(hbm_tmp);
                        }
                        else {
