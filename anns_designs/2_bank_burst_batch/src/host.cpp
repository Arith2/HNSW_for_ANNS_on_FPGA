...
// read binary file
    ...
    size_t HBM_data_size  = 64 * 1024 * 1024; // store data
...
// allocate aligned 2D vectors
    ...
    std::vector<uint32_t, aligned_allocator<uint32_t>> HBM2_data(HBM_link_size, 0);
    ...
    std::vector<uint32_t, aligned_allocator<uint32_t>> HBM3_data(HBM_query_size, 0);
    std::vector<uint32_t, aligned_allocator<uint32_t>> HBM4_data(HBM_query_size, 0);
    ...
    std::vector<int, aligned_allocator<int>> HBM5_data(DDR_data_size, 0);
    std::vector<int, aligned_allocator<int>> HBM6_data(DDR_data_size, 0);
//////////////////////////////   TEMPLATE START  //////////////////////////////
...
// store data
    ...
    for (int i = 0; i < (int) cur_element_count; i++) {
        ...
        for (int j = 0; j < 4; j++) {
            for (int k = 0; k < 16; k++) {
                uint32_t data_0 = Byte2Float(data_level0_memory_ +  i * size_data_per_element_ + offsetData_ + 128 * j + 4 * k);
                uint32_t data_1 = Byte2Float(data_level0_memory_ +  i * size_data_per_element_ + offsetData_ + 128 * j + 4 * k + 64);
                HBM0_data[i * 64 + 16 * j + k] = data_0;
                HBM1_data[i * 64 + 16 * j + k] = data_1;
            }
        }
		...
		if (element_levels_[i] != 0) {
            for (int k = 0; k < (int) (element_levels_[i]); k++) {
                ...
                for (int m = 0; m < (int) maxM_; m++) {
                    ...
                    HBM5_data[index_base + k * maxM_ + m] = high_link;
                    // if (i==enterpoint_node_) {
                    //     std::cout << "index: " << (index_base + k * maxM_ + m) << ", data: " << (high_link & 0x00ffffff ) << std::endl;
                    // }
                    // std::cout << "index: " << (index_base + index_link * 16 + k * maxM_ + m) << ", link: " << HBM5_data[index_base + index_link * 16 + k * maxM_ + m] << std::endl;
                }
                // if (i==enterpoint_node_) {
                //     std::cout << std::endl;
                // }
    ...
    for (int i = 0; i < 10000; i++) {
        ...
        for (int j = 0; j < dim; j++) {
            ...
            int index_base = i / 2;
            if (i%2==0) {
                HBM3_data[index_base * 128 + j] = query_data;
            }
            else {
                HBM4_data[index_base * 128 + j] = query_data;
            }
            // HBM5_data[index_base + j] = query_data;
        }
        // index_base += dim;
    }
    ...
    // for (int i = 0; i < 128; i++) {
    //     std::cout << HBM5_data[index_query+128*q_id+i] << ", ";
    //     if (i%16==15) {
    //         std::cout << std::endl;
    //     }
    // }
    ...
    //     for (int j = 0; j < 32; j++) {
    //         if (j < 16) {
    //             int dist_tmp = HBM0_data[enterpoint_node_*64+16*i+j] - HBM5_data[index_query+128*q_id+32*i+j];
    //             q_id_29_dist += dist_tmp * dist_tmp;
    //         }
    //         else {
    //             int dist_tmp = HBM1_data[enterpoint_node_*64+16*i+j-16] - HBM5_data[index_query+128*q_id+32*i+j];
    //             q_id_29_dist += dist_tmp * dist_tmp;
    //         }
    //     }
    // }
    ...
    // for (int i = 0; i < 4; i++) {
    //     for (int j = 0; j < 16; j++) {
    //         int sub_dist_0 = HBM0_data[enterpoint_node_*64+16*i+j] - HBM5_data[index_query+32*i+j];
    //         std::cout << sub_dist_0 << ", ";
    //         total_dist += sub_dist_0 * sub_dist_0;
    //     }
    //     std::cout << std::endl;
    //     for (int j = 0; j < 16; j++) {
    //         int sub_dist_1 = HBM1_data[enterpoint_node_*64+16*i+j] - HBM5_data[index_query+32*i+j+16];
    ...
    // int index_tmp = HBM5_data[enterpoint_node_];
    ...
    //     for (int j = 0; j < 16; j++) {
    //         int read_tmp = HBM5_data[index_tmp*16+16*i+j];
    //         if (j==0) {
    ...
    // for (int i = 0; i < 32; i++) {
    //     int read_tmp = HBM2_data[enterpoint_node_*32+i];
    //     if (i==0) {
    //         std::cout << "size: " << (read_tmp >> 24) << std::endl;
    //     }
...
//////////////////////////////   TEMPLATE START  //////////////////////////////
    ...
    // // std::cout << "HBM size: " << HBM_data_size *sizeof(uint32_t) << std::endl;
    OCL_CHECK(err, cl::Buffer buffer_HBM2_data(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE | CL_MEM_EXT_PTR_XILINX, 
            HBM_link_size *sizeof(uint32_t), &HBM2_data_ext, &err));
    // // std::cout << "HBM size: " << HBM_data_size *sizeof(uint32_t) << std::endl;
    ...
    // // std::cout << "HBM size: " << HBM_data_size *sizeof(uint32_t) << std::endl;
    OCL_CHECK(err, cl::Buffer buffer_HBM3_data(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE | CL_MEM_EXT_PTR_XILINX, 
            HBM_query_size *sizeof(uint32_t), &HBM3_data_ext, &err));
    // std::cout << "HBM 25 size: " << HBM_data_size *sizeof(uint32_t) << std::endl;
    OCL_CHECK(err, cl::Buffer buffer_HBM4_data(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE | CL_MEM_EXT_PTR_XILINX, 
            HBM_query_size *sizeof(uint32_t), &HBM4_data_ext, &err));
    // std::cout << "HBM size: " << HBM_data_size *sizeof(uint32_t) << std::endl;
    ...
    OCL_CHECK(err, cl::Buffer buffer_HBM5_data(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY | CL_MEM_EXT_PTR_XILINX, 
            DDR_data_size *sizeof(int), &HBM5_data_ext, &err));
    OCL_CHECK(err, cl::Buffer buffer_HBM6_data(context, CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY | CL_MEM_EXT_PTR_XILINX, 
            DDR_data_size *sizeof(int), &HBM6_data_ext, &err));
// ============================================================================
...
//////////////////////////////   TEMPLATE START  //////////////////////////////
	...
	OCL_CHECK(err, err = krnl_vector_add.setArg(7, enterpoint_node_));
	OCL_CHECK(err, err = krnl_vector_add.setArg(8, (int)cur_element_count));
	OCL_CHECK(err, err = krnl_vector_add.setArg(9, maxlevel_));
    OCL_CHECK(err, err = krnl_vector_add.setArg(10, ef_));
    OCL_CHECK(err, err = krnl_vector_add.setArg(11, index_link));
    OCL_CHECK(err, err = krnl_vector_add.setArg(12, q_size));
//////////////////////////////   TEMPLATE END  //////////////////////////////
// ------------------------------------------------------
...
//////////////////////////////   TEMPLATE START  //////////////////////////////
    ...
    OCL_CHECK(
        ...
        // buffer_HBM2_data, buffer_HBM3_data, buffer_HBM4_data, buffer_HBM5_data, buffer_HBM6_data, buffer_HBM7_data, 
        ...
        buffer_HBM2_data, 
        // buffer_HBM17_data, buffer_HBM18_data, buffer_HBM19_data, buffer_HBM20_data, buffer_HBM21_data, buffer_HBM22_data, buffer_HBM23_data, 
        buffer_HBM3_data, buffer_HBM4_data, 
        // buffer_HBM26_data, buffer_HBM27_data, buffer_HBM28_data, buffer_HBM29_data, buffer_HBM30_data, buffer_HBM31_data, 
        buffer_HBM5_data
        }, 0/* 0 means from host*/));
...
// --------------------------------------------------
    ...
    OCL_CHECK(err, err = q.enqueueMigrateMemObjects({buffer_HBM6_data},CL_MIGRATE_MEM_OBJECT_HOST));
    OCL_CHECK(err, err = q.finish());
// OPENCL HOST CODE AREA END
    // int DDR_count = HBM6_data[0];
// #ifndef DEBUG
//     int queue_size = 128;
// #else
//     int queue_size = 4;
// #endif
    ...
    for (int i = 0; i < q_size; i++) {
        int externalLabel = 0;
        int internal_id = HBM6_data[ddr_count+2];
        ...
        // else {
        //     std::cout << "q_id: " << i << ", hw result: " << externalLabel << ", gt: " << gt_array[100*i] << std::endl;
        // }
