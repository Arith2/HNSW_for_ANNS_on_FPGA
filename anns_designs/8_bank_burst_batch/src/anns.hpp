        ...
        for (int j = 0; j < 8; j++) {
            ...
            switch (j) {
                ...
                case 3: 
                    ...
                    break;
                case 4: 
                    query_stream_array[4].write(tmp_data);
                    break;
                case 5: 
                    query_stream_array[5].write(tmp_data);
                    break;
                case 6: 
                    query_stream_array[6].write(tmp_data);
                    break;
                default:
                    query_stream_array[7].write(tmp_data);
                    break;
            }
        }
        ...
        do {
            if (!high_layer_links_stream.empty()) {
                ...
                if (stop_flag==1) {
                    ...
                    node_id_stream.write(meta_info_32);
                    // for (int m = 0; m < 10; m++) {
                    //     #pragma HLS unroll
...
void distributeNode_id(
    hls::stream<ap_uint<32> >& node_id_stream,
    hls::stream<ap_uint<32> > node_id_stream_array[10],
    int q_size
) {
    #pragma HLS inline off
    std::cout << "distributeNode_id" << std::endl;
    bool stopLoop = false;
    ...
    for (int i = 0; i < q_size; i++) {
        ...
        do {
            if (!node_id_stream.empty()) {
                ...
                for (int k = 0; k < 10; k++) {
                    #pragma HLS unroll
                    node_id_stream_array[k].write(meta_info_32);
                }
                ...
                if (stop_flag) {
                    ...
                    for (int j = 0; j < size; j++) {
                        #pragma HLS pipeline II=1
                        ap_uint<32> node_id = node_id_stream.read();
                        for (int k = 0; k < 10; k++) {
                            #pragma HLS unroll
                            node_id_stream_array[k].write(node_id);
                        }
                    }
                }
            }
...
void readHBM_data(
    ...
    do{
        ...
        if (!node_id_stream_0.empty() | !node_id_stream_1.empty()) {
            ...
            if (meta_flag!=0xf) {
                // meta info
                // std::cout << "      mata info, channel 0" << std::endl;
                ...
                if (node_id==0x00ffffff) {
                    ...
                    for (int k = 0; k < 1; k++) {
                        ...
                        ap_uint<512> hbm_tmp = HBM_data[node_id+k]; 
                        if (channel_id==0) {
                            data_stream_0.write(hbm_tmp);
                        }
                        else {
...
// void readHBM_data(
//     hls::burst_maxi<ap_uint<512> > HBM_data, 
//     hls::stream<ap_uint<32> >& node_id_stream_0, 
//     hls::stream<ap_uint<32> >& node_id_stream_1, 
//     hls::stream<ap_uint<512> >& data_stream_0, 
//     hls::stream<ap_uint<512> >& data_stream_1, 
...
//     do{
//         if (!node_id_stream_0.empty() | !node_id_stream_1.empty()) {
//             ap_uint<32> meta_info_32 = (!node_id_stream_0.empty()) ? node_id_stream_0.read() : node_id_stream_1.read();
...
//             std::cout << "stream 0 empty: " << node_id_stream_0.empty() << ", channel id: " << channel_id << std::endl;
...
//                     switch (channel_id) {
//                         case 0:  
//                             node_id = node_id_stream_0.read();
//                             break;
//                         default: 
//                             node_id = node_id_stream_1.read();
//                             break;
//                     }
...
//                     switch (channel_id) {
//                         case 0:  
//                             data_stream_0.write(node_id_tmp);
//                             break;
//                         default: 
//                             data_stream_1.write(node_id_tmp);
//                             break;
//                     }
//                     std::cout << node_id << ", ";
//                     if (node_id==0x00ffffff) {
    ...
    do{
        ...
        if (!node_id_stream_0.empty() | !node_id_stream_1.empty()) {
            ...
            if (meta_flag!=0xf) {
                ...
                ap_uint<2> channel_id = meta_data(31, 30);
                ap_uint<1> stop_flag = meta_data(0, 0);
                std::cout << "size: " << meta_data(29, 24) << std::endl;
                if (stop_flag==1) {
                    total_count++;
                }
                else {
                    ...
                    if (cand_node==0x00ffffff) {
                        ...
                        for (int k = 0; k < 1; k++) {
                            #pragma HLS unroll
                            // ap_uint<512> hbm_tmp = HBM_data.read(); 
                            ap_uint<512> hbm_tmp = HBM_data[cand_node]; 
                            if (channel_id==0) {
                                gnd_link_stream_0.write(hbm_tmp);
                            }
                            else {
                ...
                if (node_id==0x00ffffff) {
                    ...
                    for (int k = 0; k < 1; k++) {
                        #pragma HLS unroll
                        // ap_uint<512> hbm_tmp = HBM_data.read(); 
                        ap_uint<512> hbm_tmp = HBM_data[node_id]; 
                        if (channel_id==0) {
                            gnd_link_stream_0.write(hbm_tmp);
                        }
                        else {
...
//             ap_uint<1> stop_flag = meta_info_32(0, 0);
//             int size = meta_info_32(29, 24);
//             std::cout << "level: " << level << ", cand_node: " << cand_node << ", size: " << size << std::endl;
...
//                     switch (channel_id) {
//                         case 0:  
//                             node_id = (j==0) ? cand_node : node_id_stream_0.read();
//                             break;
//                         default: 
//                             node_id = (j==0) ? cand_node : node_id_stream_1.read();
//                             break;
//                     }
//                     if (node_id==0x00ffffff) {
//                         for (int k = 0; k < 1; k++) {
                ...
                if (stop_flag==1) {
                    ...
                    for (int j = 0; j < size; j++) {     
                        ...
                        if (node_id_list[j]==0x00ffffff) {
                            ...
                            int sub_dist_out = data_array_2[0][0] + data_array_2[0][1];
...
void combineDist(
    hls::stream<ap_uint<32> > sub_stream_array[8], 
            ...
            if (!sub_stream_array[0].empty() & !sub_stream_array[3].empty() & !sub_stream_array[5].empty() & !sub_stream_array[7].empty()) {
                ...
                if (stop_flag==1) {
                    ...
                    for (int j = 0; j < size; j++) {
                        ...
                        data_array_8[0] = sub_stream_array[0].read();
                        data_array_8[1] = sub_stream_array[1].read();
                        data_array_8[2] = sub_stream_array[2].read();
                        data_array_8[3] = sub_stream_array[3].read();
