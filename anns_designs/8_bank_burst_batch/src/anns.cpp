...
// #pragma HLS INTERFACE m_axi port=HBM12_data  offset=slave bundle=gmem32
...
#pragma HLS stream variable=node_id_stream_0 depth=64
    hls::stream<ap_uint<32> > node_id_stream_1;
#pragma HLS stream variable=node_id_stream_1 depth=64
    hls::stream<ap_uint<32> > node_id_stream_array[2][10];
...
// #pragma HLS stream variable=node_id_stream_1_array depth=64
    ...
    hls::stream<ap_uint<512> > data_stream_array[2][8];
...
// #pragma HLS stream variable=data_stream_1_array depth=64
    ...
    hls::stream<ap_uint<512> > gnd_link_stream_array[2][2];
#pragma HLS stream variable=gnd_link_stream_array depth=64

    hls::stream<ap_uint<32> > sub_stream_array[2][8];
...
#pragma HLS dataflow
    ...
    loadLinks(HBM12_data, index_link_stream, cur_element_count, index_link, q_size);

    loadQuery(HBM10_data, query_stream_array[0], batch_q_size);
    loadQuery(HBM11_data, query_stream_array[1], batch_q_size);
    ...
    mergeLink(high_layer_links_stream_array[0], gnd_layer_links_stream_array[0], node_id_stream_0, batch_q_size, 0);
    mergeLink(high_layer_links_stream_array[1], gnd_layer_links_stream_array[1], node_id_stream_1, batch_q_size, 1);

    distributeNode_id(node_id_stream_0, node_id_stream_array[0], batch_q_size);
    distributeNode_id(node_id_stream_1, node_id_stream_array[1], batch_q_size);
    ...
    readHBM_link(HBM8_data, node_id_stream_array[0][8], node_id_stream_array[1][8], gnd_link_stream_array[0][0], gnd_link_stream_array[1][0], cur_element_count, q_size);
    readHBM_link(HBM9_data, node_id_stream_array[0][9], node_id_stream_array[1][9], gnd_link_stream_array[0][1], gnd_link_stream_array[1][1], cur_element_count, q_size);
    ...
    writeMem(HBM13_data, list_stream_array, q_size);
}
