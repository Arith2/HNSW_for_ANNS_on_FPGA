...
// allocate aligned 2D vectors
    ...
    std::vector<uint32_t, aligned_allocator<uint32_t>> HBM5_data(HBM_query_size, 0);
    std::vector<uint32_t, aligned_allocator<uint32_t>> HBM6_data(HBM_query_size, 0);
...
// store data
		...
		if (element_levels_[i] != 0) {
            for (int k = 0; k < (int) (element_levels_[i]); k++) {
                ...
                for (int m = 0; m < (int) maxM_; m++) {
                    ...
                    HBM7_data[index_base + k * maxM_ + m] = high_link;
                    // if (i==enterpoint_node_) {
    ...
    for (int i = 0; i < 10000; i++) {
        ...
        for (int j = 0; j < dim; j++) {
            ...
            int index_base = i / 2;
            if (i%2==0) {
                HBM5_data[index_base * 128 + j] = query_data;
            }
            else {
                HBM6_data[index_base * 128 + j] = query_data;
            }
...
//////////////////////////////   TEMPLATE START  //////////////////////////////
	...
	OCL_CHECK(err, err = krnl_vector_add.setArg(9, enterpoint_node_));
	OCL_CHECK(err, err = krnl_vector_add.setArg(10, (int)cur_element_count));
	OCL_CHECK(err, err = krnl_vector_add.setArg(11, maxlevel_));
    OCL_CHECK(err, err = krnl_vector_add.setArg(12, ef_));
    OCL_CHECK(err, err = krnl_vector_add.setArg(13, index_link));
    OCL_CHECK(err, err = krnl_vector_add.setArg(14, q_size));
//////////////////////////////   TEMPLATE END  //////////////////////////////
// ------------------------------------------------------
...
// --------------------------------------------------
    ...
    OCL_CHECK(err, err = q.enqueueMigrateMemObjects({buffer_HBM8_data},CL_MIGRATE_MEM_OBJECT_HOST));
    OCL_CHECK(err, err = q.finish());
...
// #endif
    ...
    for (int i = 0; i < q_size; i++) {
        int externalLabel = 0;
        int internal_id = HBM8_data[ddr_count+2];
