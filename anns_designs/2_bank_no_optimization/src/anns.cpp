...
#pragma HLS stream variable=node_id_stream_array depth=8
    ...
    hls::stream<ap_uint<512> > data_stream_array[2];
#pragma HLS stream variable=data_stream_array depth=8

    hls::stream<ap_uint<512> > gnd_link_stream;
#pragma HLS stream variable=gnd_link_stream depth=8
...
#pragma HLS stream variable=list_stream depth=8
    ...
    hls::stream<ap_uint<512> > sub_debug_stream_array[2];
#pragma HLS stream variable=sub_debug_stream_array depth=8
...
#pragma HLS dataflow
    ...
    loadDDR(DDR0_data, index_link_stream, query_stream_array, cur_element_count, index_link, q_size);
    ...
    mergeLink(high_layer_links_stream, gnd_layer_links_stream, node_id_stream_array, q_size);

    readHBM_data(HBM0_data, node_id_stream_array[0], data_stream_array[0], cur_element_count, q_size);
    readHBM_data(HBM1_data, node_id_stream_array[1], data_stream_array[1], cur_element_count, q_size);

    readHBM_link(HBM16_data, node_id_stream_array[2], gnd_link_stream, cur_element_count, q_size);

    subtractQuery(query_stream_array[0], data_stream_array[0], sub_stream_array[0], q_size);
    subtractQuery(query_stream_array[1], data_stream_array[1], sub_stream_array[1], q_size);

    adderTree(sub_stream_array, l2dist_stream, q_size);

    priorityQueue(l2dist_stream, ep_node_stream, cache_index_stream, list_stream, maxlevel_, enterpoint_node_, q_size);

    cacheGndLinks(gnd_link_stream, cache_index_stream, gnd_layer_links_stream, q_size);
